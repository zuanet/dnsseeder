...
DNSSeeder exposes a list of known peers to any new peer joining the Zua network via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the zuad node
...
disk in a json file, so on subsequent start ups the zuad node specified with
...
- Launch a zuad node for the DNSSeeder to connect to
...
```bash
$ git clone https://github.com/zuanet/dnsseeder $GOPATH/src/github.com/zuanet/dnsseeder
...
To create a working set-up where the DNSSeeder can provide IPs to zuad instances, set the following DNS records:
```
NAME                        TYPE        VALUE
----                        ----        -----
